...
/**
 * Will remove a service from aidpcore-node.json
...
/**
 * Will remove the Node.js service and from the aidpcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The aidpcore-node configuration directory
...
function remove(options, done) {
  ...
  var aidpcoreConfigPath = path.resolve(configPath, 'aidpcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(aidpcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a aidpcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to aidpcore-node.json
        removeConfig(aidpcoreConfigPath, service, next);
      });
    }, done
  );
}
