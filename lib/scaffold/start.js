...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for aidpd is necessary for this upgrade with the "reindex=1" aidp.conf option. \n' +
                 'There are changes necessary in both aidp.conf and aidpcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/underdarkskies/aidpcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed aidpcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './aidpcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in aidpcore-node services directory
    ...
    if (servicePackage.aidpcoreNode) {
      serviceModule = service.name + '/' + servicePackage.aidpcoreNode;
    }
 ...
 * @param {Node} node - The Aidpcore Node instance
 */
