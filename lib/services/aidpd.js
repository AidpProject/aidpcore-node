...
/**
 * Provides a friendly event driven API to aidpd in Node.js. Manages starting and
 * stopping aidpd as a child process for application support, as well as connecting
 * to multiple aidpd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Aidpcoin(options) {
  if (!(this instanceof Aidpcoin)) {
    return new Aidpcoin(options);
  }
...
util.inherits(Aidpcoin, Service);

Aidpcoin.dependencies = [];

Aidpcoin.DEFAULT_MAX_TXIDS = 1000;
Aidpcoin.DEFAULT_MAX_HISTORY = 50;
Aidpcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Aidpcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Aidpcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Aidpcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Aidpcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Aidpcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Aidpcoin.DEFAULT_REINDEX_INTERVAL = 10000;
Aidpcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Aidpcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Aidpcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Aidpcoin.DEFAULT_CONFIG_SETTINGS = {
  assetindex: 1,
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
...
Aidpcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Aidpcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Aidpcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Aidpcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Aidpcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Aidpcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Aidpcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Aidpcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Aidpcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Aidpcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Aidpcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Aidpcoin.prototype._initCaches = function() {
...
Aidpcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Aidpcoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Aidpcoin.prototype.getPublishEvents = function() {
  return [
    {
...
Aidpcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'aidpd/' + name, 'total:', this.subscriptions[name].length);
};

Aidpcoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'aidpd/' + name, 'total:', this.subscriptions[name].length);
};

Aidpcoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (aidpcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'aidpd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Aidpcoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'aidpd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Aidpcoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Aidpcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'aidpd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Aidpcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'aidpd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Aidpcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Aidpcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Aidpcoin.DEFAULT_CONFIG_SETTINGS;
...
Aidpcoin.prototype._parseAidpcoinConf = function(configPath) {
  var options = {};
...
Aidpcoin.prototype._expandRelativeDatadir = function() {
...
Aidpcoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in aidpd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in aidpd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in aidpd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './aidp.conf');

  log.info('Using aidpcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseAidpcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseAidpcoinConf(networkConfigPath));
  }
...
Aidpcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of aidpcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of aidpcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of aidpcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to aidpd from aidpcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from aidpd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from aidpd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in aidp.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that aidpd is undergoing a reindex. ' +
             ...
             'of aidpcore-node services will start.');
    node._reindex = true;
  }
};

Aidpcoin.prototype._resetCaches = function() {
...
Aidpcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Aidpcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Aidpcoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Aidpcoin Daemon Ready');
          callback();
        });
      });
...
Aidpcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === aidpcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

Aidpcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === aidpcore.Networks.testnet) {
    networkPath = 'testnet3/aidp.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/aidp.conf';
    }
  }
  return networkPath;
};

Aidpcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === aidpcore.Networks.testnet) {
...
Aidpcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('aidpd/hashblock', message.toString('hex'));
    }
  }

};

Aidpcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Aidpcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Aidpcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Aidpcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Aidpcoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Aidpcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('aidpd/addressbalance', {
                    address: address,
                    txid: txid,
...
Aidpcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('aidpd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Aidpcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = aidpcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('aidpd/rawtransaction', message.toString('hex'));
    }
...
Aidpcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Aidpcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Aidpcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Aidpcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Aidpcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Aidpcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Aidpcoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Aidpcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Aidpcoin.prototype._stopSpawnedAidpcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/aidpd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned aidpcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean aidpcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Aidpcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedAidpcoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting aidpcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Aidpcoin process unexpectedly exited with code:', code);
        log.warn('Restarting aidpcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Aidpcoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn aidpd.'));
      }
...
Aidpcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to aidpd.'));
    }
 ...
 */
Aidpcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Aidpcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Aidpcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, aidpcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Aidpcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = aidpcore.Script.fromAddress(delta.address);
    return {
...
Aidpcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Aidpcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Aidpcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in aidpcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Aidpcoin.prototype._getConfirmationsDetail = function(transaction) {
...
Aidpcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Aidpcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Aidpcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A aidpcoin transaction id
 * @param {Function} callback
 */
Aidpcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Aidpcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof aidpcore.Address) {
...
Aidpcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
Aidpcoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Aidpcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getRawBlock = function(blockArg, callback) {
...
Aidpcoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Aidpcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Aidpcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = aidpcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Aidpcoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
Aidpcoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Aidpcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Aidpcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.getInfo = function(callback) {
  var self = this;
...
Aidpcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('aidpd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('aidpd process did not exit'));
      }
...
Aidpcoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
Aidpcoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
Aidpcoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
Aidpcoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
Aidpcoin.prototype.getPeerInfo = function(callback) {
    var self = this;
