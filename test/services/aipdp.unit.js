...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/aidp.conf')));
var AidpcoinService = proxyquire('../../lib/services/aidpd', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultAidpcoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.aidp.conf'), 'utf8');

describe('Aidpcoin Service', function() {
  ...
  var baseConfig = {
    node: {
      network: aidpcore.Networks.testnet
    },
    spawn: {
      datadir: 'testdir',
      exec: 'testpath'
  ...
  describe('@constructor', function() {
    ...
    it('will create an instance without `new`', function() {
      var aidpd = AidpcoinService(baseConfig);
      should.exist(aidpd);
    });
    it('will init caches', function() {
      var aidpd = new AidpcoinService(baseConfig);
      should.exist(aidpd.utxosCache);
      should.exist(aidpd.txidsCache);
      should.exist(aidpd.balanceCache);
      should.exist(aidpd.summaryCache);
      should.exist(aidpd.transactionDetailedCache);

      should.exist(aidpd.transactionCache);
      should.exist(aidpd.rawTransactionCache);
      should.exist(aidpd.blockCache);
      should.exist(aidpd.rawBlockCache);
      should.exist(aidpd.blockHeaderCache);
      should.exist(aidpd.zmqKnownTransactions);
      should.exist(aidpd.zmqKnownBlocks);
      should.exist(aidpd.lastTip);
      should.exist(aidpd.lastTipTimeout);
    });
    it('will init clients', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.nodes.should.deep.equal([]);
      aidpd.nodesIndex.should.equal(0);
      aidpd.nodes.push({client: sinon.stub()});
      should.exist(aidpd.client);
    });
    it('will set subscriptions', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.subscriptions.should.deep.equal({
        address: {},
		balance: {},
        rawtransaction: [],
        hashblock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._initDefaults({transactionConcurrency: 10});
      aidpd.transactionConcurrency.should.equal(10);
      aidpd._initDefaults({});
      aidpd.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      AidpcoinService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var methods = aidpd.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var events = aidpd.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('aidpd/rawtransaction');
      events[0].scope.should.equal(aidpd);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('aidpd/hashblock');
      events[1].scope.should.equal(aidpd);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('aidpd/addresstxid');
      events[2].scope.should.equal(aidpd);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('aidpd/addressbalance');
      events[3].scope.should.equal(aidpd);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.subscribe = sinon.stub();
      aidpd.unsubscribe = sinon.stub();
      var events = aidpd.getPublishEvents();

      events[0].subscribe('test');
      aidpd.subscribe.args[0][0].should.equal('rawtransaction');
      aidpd.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      aidpd.unsubscribe.args[0][0].should.equal('rawtransaction');
      aidpd.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      aidpd.subscribe.args[1][0].should.equal('hashblock');
      aidpd.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      aidpd.unsubscribe.args[1][0].should.equal('hashblock');
      aidpd.unsubscribe.args[1][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var emitter = {};
      aidpd.subscribe('hashblock', emitter);
      aidpd.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      aidpd.subscribe('rawtransaction', emitter2);
      aidpd.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      aidpd.subscribe('hashblock', emitter1);
      aidpd.subscribe('hashblock', emitter2);
      aidpd.subscribe('hashblock', emitter3);
      aidpd.subscribe('hashblock', emitter4);
      aidpd.subscribe('hashblock', emitter5);
      aidpd.subscriptions.hashblock.length.should.equal(5);

      aidpd.unsubscribe('hashblock', emitter3);
      aidpd.subscriptions.hashblock.length.should.equal(4);
      aidpd.subscriptions.hashblock[0].should.equal(emitter1);
      aidpd.subscriptions.hashblock[1].should.equal(emitter2);
      aidpd.subscriptions.hashblock[2].should.equal(emitter4);
      aidpd.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      aidpd.subscriptions.hashblock= [emitter1];
      aidpd.unsubscribe('hashblock', emitter3);
      aidpd.subscriptions.hashblock.length.should.equal(1);
      aidpd.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      aidpd.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(aidpd.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var emitter = new EventEmitter();
      aidpd.subscribeAddress(emitter, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will handle multiple address subscribers', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      aidpd.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      aidpd.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
      aidpd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      aidpd.unsubscribeAddress(emitter1);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      aidpd.unsubscribeAddress(emitter1, ['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter2];
      aidpd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      aidpd.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      aidpd.unsubscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.not.exist(aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      aidpd.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'] = [emitter1, emitter2];
      sinon.spy(aidpd, 'unsubscribeAddressAll');
      aidpd.unsubscribeAddress(emitter1);
      aidpd.unsubscribeAddressAll.callCount.should.equal(1);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      aidpd.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      aidpd.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'] = [emitter1, emitter2];
      aidpd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      aidpd.subscriptions.address['rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh'] = [emitter1];
      aidpd.unsubscribeAddress(emitter1);
      aidpd.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      aidpd.subscriptions.address['RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'].length.should.equal(1);
      aidpd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(aidpd.subscriptions.address['rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var config = aidpd._getDefaultConfig();
      config.should.equal(defaultAidpcoinConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a aidp.conf file', function() {
      var TestAidpcoin = proxyquire('../../lib/services/aidpd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var aidpd = new TestAidpcoin(baseConfig);
      aidpd.options.spawn.datadir = '/tmp/.aidp';
      var node = {};
      aidpd._loadSpawnConfiguration(node);
      should.exist(aidpd.spawn.config);
      aidpd.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: aidpcore.Networks.testnet,
          configPath: '/tmp/.aidpcore/aidpcore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var aidpd = new TestAidpcoin(config);
      aidpd.options.spawn.datadir = './data';
      var node = {};
      aidpd._loadSpawnConfiguration(node);
      aidpd.options.spawn.datadir.should.equal('/tmp/.aidpcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestAidpcoin = proxyquire('../../lib/services/aidpd', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badaidp.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var aidpd = new TestAidpcoin(baseConfig);
      (function() {
        aidpd._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(aidpcore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the aidp.conf file', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      aidpd._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        aidpd.transactionDetailedCache.set(keys[i], {});
        aidpd.utxosCache.set(keys[i], {});
        aidpd.txidsCache.set(keys[i], {});
        aidpd.balanceCache.set(keys[i], {});
        aidpd.summaryCache.set(keys[i], {});
      }
      aidpd._resetCaches();
      should.equal(aidpd.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(aidpd.utxosCache.get(keys[0]), undefined);
      should.equal(aidpd.txidsCache.get(keys[0]), undefined);
      should.equal(aidpd.balanceCache.get(keys[0]), undefined);
      should.equal(aidpd.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      aidpd._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        aidpd.nodes[0].client.getInfo.callCount.should.equal(1);
        aidpd.nodes[1].client.getInfo.callCount.should.equal(1);
        aidpd.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      aidpd._tryAllClients(function(client, next) {
        ...
        aidpd.nodes[1].client.getInfo.callCount.should.equal(1);
        aidpd.nodes[2].client.getInfo.callCount.should.equal(1);
        aidpd.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      aidpd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      aidpd.nodes.push({
        client: {
      ...
      aidpd._initChain(function() {
        log.info.callCount.should.equal(1);
        aidpd.getRawBlock.callCount.should.equal(1);
        aidpd.getRawBlock.args[0][0].should.equal('genesishash');
        aidpd.height.should.equal(5000);
        aidpd.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      aidpd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      aidpd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      aidpcore.Networks.disableRegtest();
      baseConfig.node.network = aidpcore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      var config = {
        node: {
          network: aidpcore.Networks.livenet
        },
        spawn: {
          datadir: 'testdir',
          exec: 'testpath'
      ...
      var aidpd = new AidpcoinService(config);
      aidpd._getDefaultConf().rpcport.should.equal(8766);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var aidpd = new AidpcoinService(config);
      aidpd._getDefaultConf().rpcport.should.equal(18766);
    });
    it('will get default rpc port for regtest', function() {
      aidpcore.Networks.enableRegtest();
      var config = {
        node: {
          network: aidpcore.Networks.testnet
        },
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var aidpd = new AidpcoinService(config);
      should.equal(aidpd._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var aidpd = new AidpcoinService(config);
      aidpd._getNetworkConfigPath().should.equal('testnet3/aidp.conf');
    });
    it('will get default rpc port for regtest', function() {
      aidpcore.Networks.enableRegtest();
      var config = {
      ...
      var aidpd = new AidpcoinService(config);
      aidpd._getNetworkConfigPath().should.equal('regtest/aidp.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.node.network = aidpcore.Networks.testnet;
      aidpd._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.node.network = aidpcore.Networks.testnet;
      aidpcore.Networks.enableRegtest();
      aidpd._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.node.network = aidpcore.Networks.livenet;
      aidpcore.Networks.enableRegtest();
      should.equal(aidpd._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      aidpd._rapidProtectedUpdateTip = sinon.stub();
      aidpd.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      aidpd._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      aidpd._rapidProtectedUpdateTip = sinon.stub();
      aidpd._zmqBlockHandler(node, message);
      aidpd._rapidProtectedUpdateTip.callCount.should.equal(1);
      aidpd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      aidpd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      aidpd._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      aidpd.subscriptions.hashblock.push(emitter);
      emitter.on('aidpd/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        aidpd._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub();
      ...
      aidpd._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      aidpd.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      aidpd.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      aidpd.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub();
      aidpd._resetCaches = sinon.stub();
      aidpd.on('tip', function(height) {
        aidpd._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        aidpd.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      aidpd.on('tip', function() {
        aidpd.syncPercentage.callCount.should.equal(0);
        done();
      });
      aidpd._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using aidpcore.Transaction', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var wif = 'L2Gkw3kKJ6N24QcDuH4XDqt9cTqsKTVNDGz1CRZhk9cq4auDUbJy';
      var privkey = aidpcore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(aidpcore.Networks.testnet);
      var outputAddress = aidpcore.Address('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br');
      var tx = aidpcore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: aidpcore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = aidpd._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(aidpcore.Transaction.Output({
        script: aidpcore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = aidpd._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var tx = aidpcore.Transaction();
      var address = aidpcore.Address('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br');
      tx.addOutput(aidpcore.Transaction.Output({
        script: aidpcore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(aidpcore.Transaction.Output({
        script: aidpcore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      aidpd._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      aidpd.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('aidpd/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      aidpd._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      aidpd.subscriptions.rawtransaction.push(emitter);
      emitter.on('aidpd/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      aidpd._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      aidpd.subscriptions.rawtransaction.push(emitter);
      emitter.on('aidpd/rawtransaction', function() {
        done();
      });
      var node = {};
      aidpd._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._updateTip = sinon.stub();
      aidpd._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      aidpd.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      aidpd._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        aidpd._updateTip.callCount.should.equal(11);
        aidpd._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      aidpd._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        aidpd._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      aidpd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        aidpd._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      aidpd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        aidpd._zmqBlockHandler.callCount.should.equal(0);
        aidpd._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var aidpd = new AidpcoinService(baseConfig);
      var node = {};
      aidpd._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      aidpd._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      aidpd._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      aidpd._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestAidpcoinService = proxyquire('../../lib/services/aidpd', {
        fs: {
          readFile: readFile
        }
      });
      var aidpd = new TestAidpcoinService(baseConfig);
      aidpd.spawnStopTime = 1;
      aidpd._process = {};
      aidpd._process.kill = sinon.stub();
      aidpd._stopSpawnedAidpcoin(function(err) {
        if (err) {
          return done(err);
        }
        aidpd._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      aidpd._process.kill = sinon.stub().throws(error2);
      aidpd._stopSpawnedAidpcoin(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._loadSpawnConfiguration = sinon.stub();
      aidpd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      aidpd._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedAidpcoin', function() {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._loadSpawnConfiguration = sinon.stub();
      aidpd._stopSpawnedAidpcoin = sinon.stub().callsArgWith(0, new Error('test'));
      aidpd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var aidpd = new TestAidpcoinService(config);
      aidpd.spawn = {};
      aidpd._loadSpawnConfiguration = sinon.stub();
      aidpd._stopSpawnedAidpcoin = sinon.stub().callsArgWith(0, null);
      aidpd.node.stopping = true;
      aidpd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      aidpd._loadSpawnConfiguration = sinon.stub();
      aidpd.spawn = {};
      aidpd.spawn.exec = 'testexec';
      aidpd.spawn.configPath = 'testdir/aidp.conf';
      aidpd.spawn.datadir = 'testdir';
      aidpd.spawn.config = {};
      aidpd.spawn.config.rpcport = 20001;
      aidpd.spawn.config.rpcuser = 'aidpcoin';
      aidpd.spawn.config.rpcpassword = 'password';
      aidpd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

      aidpd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      aidpd._initZmqSubSocket = sinon.stub();
      aidpd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      aidpd._checkReindex = sinon.stub().callsArgWith(1, null);
      aidpd._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/aidp.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        aidpd._loadTipFromNode.callCount.should.equal(1);
        aidpd._initZmqSubSocket.callCount.should.equal(1);
        should.exist(aidpd._initZmqSubSocket.args[0][0].client);
        aidpd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        aidpd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(aidpd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn aidpd spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestAidpcoinService = proxyquire('../../lib/services/aidpd', {
        fs: {
      ...
      aidpd._loadSpawnConfiguration = sinon.stub();
      aidpd.spawn = {};
      aidpd.spawn.exec = 'aidpd';
      aidpd.spawn.datadir = '/tmp/aidpcoin';
      aidpd.spawn.configPath = '/tmp/aidpcoin/aidp.conf';
      aidpd.spawn.config = {};
      aidpd.spawnRestartTime = 1;
      aidpd._loadTipFromNode = sinon.stub().callsArg(1);
      aidpd._initZmqSubSocket = sinon.stub();
      aidpd._checkReindex = sinon.stub().callsArg(1);
      aidpd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      aidpd._stopSpawnedAidpcoin = sinon.stub().callsArg(0);
      sinon.spy(aidpd, '_spawnChildProcess');
      aidpd._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            aidpd._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      aidpd._spawnChildProcess(function(err) {
        ...
        aidpd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        aidpd.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn aidpd spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestAidpcoinService = proxyquire('../../lib/services/aidpd', {
        fs: {
      ...
      aidpd._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            aidpd._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      aidpd.spawn.config.rpcpassword = 'password';
      aidpd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      aidpd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      aidpd._spawnChildProcess(function(err) {
        aidpd._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      aidpd._initZmqSubSocket = sinon.stub();
      aidpd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      aidpd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var aidpd = new AidpcoinService(config);
      aidpd.node.stopping = true;
      aidpd.startRetryInterval = 100;
      aidpd._loadTipFromNode = sinon.stub();
      aidpd._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        aidpd._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      aidpd.startRetryInterval = 1;
      var config = {};
      aidpd._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        aidpd._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      aidpd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      aidpd._connectProcess(config, function(err, node) {
        should.not.exist(err);
        aidpd._loadTipFromNode.callCount.should.equal(1);
        aidpd._initZmqSubSocket.callCount.should.equal(1);
        aidpd._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      ...
      aidpd.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Aidpcoin configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      aidpd.options = {
        connect: [
          {}
        ]
      ...
      aidpd.start(function(err) {
        aidpd._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      var node = {};
      aidpd._initChain = sinon.stub().callsArg(0);
      aidpd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      aidpd.options = {
        spawn: {}
      };
      aidpd.start(function(err) {
        should.not.exist(err);
        aidpd.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      aidpd._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      aidpd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      aidpd.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      aidpd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      aidpd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      aidpd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      aidpd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      aidpd.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var args = aidpd._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var args = aidpd._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN';
      var options = {};
      aidpd.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN';
      var options = {};
      aidpd.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        aidpd.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      aidpd.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = aidpd._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = aidpd._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = aidpd._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      aidpd._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        aidpd._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      aidpd.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      aidpd.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      aidpd.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'RM1FZ5Q4sKxsM1a97dLoUrjZYHZ7B6MKja';
      aidpd.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        aidpd.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
          ...
          aidpd.getAddressTxids(address, {queryMempoolOnly: true}, function(err, txids) {
            if (err) {
              return done(err);
            }
            getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.height = 10;
      var confirmations = aidpd._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var result = {};
      aidpd._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var result = {};
      aidpd._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var result = {};
      aidpd._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var aidpd = new AidpcoinService(baseConfig);
      var result = {};
      aidpd._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = aidpd._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      aidpd.height = 300;
      var addresses = {};
      aidpd._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      aidpd._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      aidpd._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from aidpcore addresses', function() {
      var addresses = [
        aidpcore.Address('RJYZeWxr1Ly8YgcvJU1qD5MR9jUtk14HkN'),
        aidpcore.Address('rAfsiNFiHsvDwEA1JsaE9Qmad5CgPVbELh'),
      ];
      var aidpd = new AidpcoinService(baseConfig);
      var strings = aidpd._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var aidpd = new AidpcoinService(baseConfig);
      (function() {
        aidpd._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      aidpd.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      aidpd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      aidpd.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.maxAddressesQuery = 100;
      aidpd.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      aidpd.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      aidpd.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      aidpd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      aidpd.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      aidpd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      aidpd.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(aidpd, '_paginateTxids');
      aidpd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      aidpd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      aidpd.getAddressSummary(address, options, function(err, summary) {
        aidpd._paginateTxids.callCount.should.equal(1);
        aidpd._paginateTxids.args[0][1].should.equal(0);
        aidpd._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      aidpd.getAddressSummary(address, options, function(err, summary) {
        ...
        aidpd.getAddressTxids.callCount.should.equal(1);
        aidpd.getAddressBalance.callCount.should.equal(1);
        aidpd.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          aidpd.getAddressTxids.callCount.should.equal(1);
          aidpd.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      aidpd.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      aidpd._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '3NbU8XzUgKyuCgYgZEKsBtUvkTm2r7Xgwj';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      aidpd.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      aidpd.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      aidpd.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      aidpd.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will getblock as aidpcore object from height', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      aidpd.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(aidpcore.Block);
        done();
      });
    });
    it('will getblock as aidpcore object', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      aidpd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      aidpd.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      aidpd.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      aidpd.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      aidpd.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      aidpd.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      aidpd.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      aidpd.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      aidpd._maybeGetBlockHash('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      aidpd._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      aidpd._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      aidpd.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      aidpd.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      aidpd.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = aidpcore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      aidpd.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      aidpd.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      aidpd.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = aidpcore.Transaction();
      (function() {
        aidpd.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      aidpd.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      aidpd.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(aidpcore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      aidpd.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      aidpd.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
	...
	it('will set height to -1 if missing height and get time from raw transaction', function(done) {
		var aidpd = new AidpcoinService(baseConfig);
		sinon.spy(aidpd, '_tryAllClients');
		var rawTransaction = JSON.parse((JSON.stringify(rpcRawTransaction)));
		...
		aidpd.getDetailedTransaction(txid, function(err, tx) {
			should.exist(tx);
			aidpd._tryAllClients.callCount.should.equal(1);
			getRawTransaction.callCount.should.equal(1);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      aidpd.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      aidpd.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      aidpd.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      aidpd.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      aidpd.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.spawn = {};
      aidpd.spawn.process = new EventEmitter();
      aidpd.spawn.process.kill = sinon.stub();
      aidpd.stop(done);
      aidpd.spawn.process.kill.callCount.should.equal(1);
      aidpd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      aidpd.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      aidpd.spawn.process.kill.callCount.should.equal(1);
      aidpd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      aidpd.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var aidpd = new AidpcoinService(baseConfig);
      aidpd.shutdownTimeout = 300;
      aidpd.spawn = {};
      aidpd.spawn.process = new EventEmitter();
      aidpd.spawn.process.kill = sinon.stub();
      aidpd.stop(function(err) {
