...
describe('Aidpcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/aidpd');
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/aidp.conf'),
      rpcuser: 'aidpcoin',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/aidp.conf'),
      rpcuser: 'aidpcoin',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/aidp.conf'),
      rpcuser: 'aidpcoin',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 aidpd daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three aidpd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = aidpcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.aidpd.once('tip', function(height) {
